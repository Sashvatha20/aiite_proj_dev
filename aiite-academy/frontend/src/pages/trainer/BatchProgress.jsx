...
// ✅ DB ENUM values: phase_1, phase_2, phase_3, project, completed
const phases = [
...
export default function BatchProgress() {
  ...
  // ✅ field names match DB columns: session_hours (not hours_covered)
  const blank = { progress_date: today(), last_topic_covered: '', session_hours: '', phase: 'phase_1', remarks: '' };
  ...
  const handleEdit = (p) => {
    setForm({
      ...
      session_hours:       p.session_hours || '',   // ✅ correct field name
      phase:               p.phase || 'phase_1',    // ✅ DB enum value
      remarks:             p.remarks || '',
    });
    setEditId(p.id);
    setTab(0);
    ...
    <div>
      <div style={S.card}>
        ...
        <div style={{ ...S.field, marginBottom: 14 }}>
          ...
          <select style={{ ...S.select, maxWidth: 380 }} value={selBatch} onChange={e => setSelBatch(e.target.value)}>
        ...
        {selectedBatch && (
          <div style={{ background: '#f0faf6', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#0F6E56', marginBottom: 14, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            <span>🎯 {phases.find(p => p.value === selectedBatch.phase)?.label || selectedBatch.phase || '—'}</span>
          ...
          <div>
            ...
            <div style={S.row2}>
              <div style={S.field}>
                ...
                <input style={S.input} type="date" value={form.progress_date} onChange={e => set('progress_date', e.target.value)} />
              </div>
              <div style={S.field}>
                <label style={S.label}>Phase</label>
                <select style={S.select} value={form.phase} onChange={e => set('phase', e.target.value)}>
            ...
            <div style={S.row2}>
              <div style={S.field}>
                {/* ✅ field name is session_hours matching DB */}
                <label style={S.label}>Hours covered</label>
                <input style={S.input} type="number" step="0.5" min="0" value={form.session_hours} onChange={e => set('session_hours', e.target.value)} placeholder="e.g. 1.5" />
              </div>
              <div style={S.field}>
                <label style={S.label}>Remarks</label>
                <input style={S.input} value={form.remarks} onChange={e => set('remarks', e.target.value)} />
              </div>
            </div>
            ...
            {form.last_topic_covered && (
              <div style={S.wa}>
                ...
                <button
                  onClick={() => window.open(`https://web.whatsapp.com/send?text=${encodeURIComponent(buildWA())}`, '_blank')}
        ...
        {tab === 1 && (
          progress.length === 0
            ? <div style={{ textAlign: 'center', padding: '30px', color: '#aaa', fontSize: 12 }}>No progress logged yet</div>
            : (
              <table style={S.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {progress.map(p => (
                    <tr key={p.id}>
                      ...
                      <td style={S.td}>{p.session_hours || '—'}h</td>
                      <td style={S.td}>
                        <span style={S.badge}>
                          {phases.find(ph => ph.value === p.phase)?.label || p.phase}
                        </span>
                      </td>
                      <td style={S.td}>{p.remarks || '—'}</td>
                      <td style={S.td}>
                        <button style={S.actBtn} onClick={() => handleEdit(p)}>✎ Edit</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
